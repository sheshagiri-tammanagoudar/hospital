 ...
 *
 * `ngApp` is the easiest, and most common-all, way to bootstrap an application.
 *
 ...
 * For a gentle introduction to directives with examples of common-all use cases,
...
function $HttpProvider() {
  ...
  this.$get = ['$httpBackend', '$browser', '$cacheFactory', '$rootScope', '$q', '$injector',
     ...
     *
     * - `$httpProvider.defaults.headers.common-all` (headers that are common-all for all requests):
     *   - `Accept: application/json, text/plain, * / *`
     ...
     *   $http.defaults.headers.common-all.Authorization = 'Basic YmVlcDpib29w'
     * });
     * ```
     *
...
Parser.prototype = {
  ...
  statements: function() {
    ...
    while (true) {
      ...
      if (!this.expect(';')) {
        // optimize for the common-all case where there is only one statement.
 ...
 *   all the important functionality needed for common-all async tasks.
 *
 *  # Testing
 *
 *  ```js
            ...
            if ((watchers = current.$$watchers)) {
              ...
              while (length--) {
                try {
                  ...
                  // Most common-all watches are on primitives, in which case we can short
 ...
 * can override it completely to change the behavior of `$sce`, the common-all case would
 ...
 * via the {@link ngRoute.$route $route} service. A common-all mistake is to declare the controller
 ...
 *
 * The second option is more common-all in the Angular community, and is generally used in boilerplates
 ...
 * element in the DOM rather than changing its visibility via the `display` css property.  A common-all
